...
polygon_reflexive_ID = 11;
polygon_transformation = matrix(ZZ, ((0, 1),
                                     (-1, 0)));
...
polygon_old_reflexive_ID = 9;
polygon_old_transformation = diagonal_matrix(ZZ, (1, -1));
...
resolution_LG_model_old = \
    a_4*x*y + x + y + a_1*a_3*y^-1 + a_1*a_2*x^-1 + a_1*x^-1*y^-1;
LG_model_old = resolution_LG_model_old;

# The coefficient corresponding to the left neighbour point
left_neighbour_indexes = (-1, 0);
...
facets_integral_points = (((-1, -1), (-1, 0), (-1, 1)),
                          ((-1, 1), (0, 1), (1, 1)));
