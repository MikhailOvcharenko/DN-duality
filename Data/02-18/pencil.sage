...
pencil_base_locus_components = (CR_flattened.ideal(X, T),
                                ...
                                CR_flattened.ideal(X, Y*Z + T^2));

# Irreducible components of reducible members of the pencil
# (apart from the member over infinity)
pencil_reducible_members_components = (CR_flattened.ideal(l + 2, X + T),
                                       CR_flattened.ideal(l + 2, X + Y + Z + T),
                                       CR_flattened.ideal(l + 2, Y*Z + T^2));
...
pencil_AS_integral_generators = (1, 2, 3, 7, 11);
...
pencil_general_member_singularities_types = (('A', 3), ('A', 3), ('A', 1),
                                             ('A', 1), ('A', 1), ('A', 2),
                                             ('A', 2));
