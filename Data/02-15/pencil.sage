...
pencil_base_locus_components = (CR_flattened.ideal(X, T),
                                ...
                                CR_flattened.ideal(Z, T*(X + Y) + X*Y));
...
pencil_reducible_members_components = \
    (CR_flattened.ideal(l + 1, X + Y + Z),
     CR_flattened.ideal(l + 1, X*Y*(Z + T) + T^2*(X + Y + Z)));
...
pencil_AS_integral_generators = (1, 2, 3, 4, 5, 6, 8);
