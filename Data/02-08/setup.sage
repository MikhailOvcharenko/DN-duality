...
fano_threefold_family_no = 8;
fano_threefold_picard_lattice = matrix(ZZ, ((-2, 0),
                                            (0, 4)));
...
LG_model = x*y*z^-1 + 2*x + 2*y + x*y^-1*z + 2*z + x^-1*y*z + 2*x*z^-1 + \
    2*x*y^-1 + x*y^-1*z^-1 + 2*z^-1 + 2*y^-1 + 2*x^-1 + 2*y^-1*z^-1 + \
    x^-1*y^-1*z^-1;
LG_model_polytope_PALP_ID = 1969;
LG_model_period_sequence_GRDB_ID = 144;
