...
pencil_base_locus_components = (CR_flattened.ideal(Z, T),
                                CR_flattened.ideal(Z, X + Y),
                                CR_flattened.ideal(T, Y + Z),
                                CR_flattened.ideal(X, Y*(Z + T) + Z^2),
                                CR_flattened.ideal(Y, Z^2 + X*T),
                                CR_flattened.ideal(Z, Y*(X + T) + X*T),
                                CR_flattened.ideal(T, Y*(X + Z) + Z^2));
...
pencil_reducible_members_components = \
    (CR_flattened.ideal(l + 2, Z*(Y + Z) + T*(X + Y)),
     CR_flattened.ideal(l + 2, Z*(Y + Z) + T*(X + Y) + X*Y));
