...
pencil_base_locus_components = (CR_flattened.ideal(X, Y),
                                ...
                                CR_flattened.ideal(Z, Y*(X + Y) + X*T));
...
pencil_reducible_members_components = \
    (CR_flattened.ideal(l + 1, X + Z),
     CR_flattened.ideal(l + 1, Z + T),
     CR_flattened.ideal(l + 1, X*(Y + T) + Y^2));
...
pencil_AS_integral_generators = (3, 4, 5, 6, 11);
