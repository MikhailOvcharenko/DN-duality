...
pencil_base_locus_components = (CR_flattened.ideal(X, Y),
                                ...
                                CR_flattened.ideal(Z, X*Y + X*T + Y*T),
                                CR_flattened.ideal(T, X*Y + X*Z + Y*Z));
...
pencil_reducible_members_components = \
    (CR_flattened.ideal(l + 6, Z + T),
     CR_flattened.ideal(l + 6, X + Y + Z + T),
     CR_flattened.ideal(l + 6, (X + Y)*(Z + T) + X*Y));
